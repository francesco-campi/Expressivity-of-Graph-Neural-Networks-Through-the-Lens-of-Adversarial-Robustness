...
def Elist(graph):
    "returns a list with the neighbours of each node of the graph"
    E_list = []
...
def helper(graph, subtensor_length = 4, full_permutation = False):
    ...
    for i in range(num_of_nodes):
        ...
        this_node_egonet_seq = np.array(this_node_egonet_seq, dtype=object)
        egonet_seq_graph.append(this_node_egonet_seq)

    #egonet_seq_graph = np.array(egonet_seq_graph, dtype=object)
    return egonet_seq_graph

class DglSyntheticDataset(object):
    def __init__(self, dataset_path,  dataset_name, task = 'star', subtensor_length = 4, full_permutation = False):
        ...
        self.lrp_save_path = os.path.join(self.dataset_path, "lrp_save")


        
        if full_permutation:
            self.lrp_save_file = 'lrp_light_len_' + self.dataset_name + '_full'
        else:
            self.lrp_save_file = 'lrp_light_len_' + self.dataset_name
    ...
    def load_graphs(self):
        ...
        glist, all_labels = load_graphs(os.path.join(self.dataset_path, self.dataset_name + ".bin"))
    ...
    def LRP_preprocess(self):
        ...
        if len(self.lrp_egonet_seq) == 0:
            ...
            for g in tqdm(self.graphs):
                ...
                #print(self.lrp_egonet_seq[-1].shape)
                #print(self.lrp_egonet_seq[-1])
        
            self.lrp_egonet_seq = np.array(self.lrp_egonet_seq, dtype=object)
...
class GraphDataset(Dataset):
    ...
    def __init__(self, dataset_path: str, task: str, in_channels: int = 0, use):#,  transform: Callable = None, pre_transform: Callable = None, pre_filter: Callable = None):
        super().__init__()
        self.task = task
        self.dataset_path = dataset_path
    ...
    def _process(self):
        ...
        for graph in graphs:
            ...
            self.data.append(Data(x, edge_index).coalesce())
